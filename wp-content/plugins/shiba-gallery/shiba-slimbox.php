...
class Shiba_Gallery_SlimBox {
	...
	function render($images, $args) {
		...
		foreach ( $images as $image ) {		
			...
			$imgStr .= "<div class=\"slimbox-thumb shiba-outer\">";
			if (is_array($full)) 
				$imgStr .= "<a href=\"{$full[0]}\" rel=\"lightbox-shiba{$lightNum}\" title='{$img_caption}'>";
			else	
				$imgStr .= "<a href=\"{$full_empty}\" rel=\"lightbox-shiba{$lightNum}\" title='{$img_caption}'>";
			$imgStr .= "<img src=\"{$img[0]}\" width=\"{$img[1]}\" height=\"{$img[2]}\" style=\"padding:{$padding}\" alt=\"{$title}\" title=\"{$title}\" class=\"shiba-stage\"/>";
			$imgStr .= "</a></div>\n";
			$j++;
		}
